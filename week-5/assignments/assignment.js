...
const renderTable = () => {
 ...
 if (withFilters) {
   ...
   data = person.filter(({ first_name }) => first_name.toLowerCase().includes(nameTerm));
   if ( 0 == data.length)
   {
     data = person.filter(({ last_name }) => last_name.toLowerCase().includes(nameTerm));
     if ( 0 == data.length)
     {
         data = person.filter(({ email }) => email.toLowerCase().includes(nameTerm));
         if ( 0 == data.length)
         {
             data = person.filter(({ gender }) => gender.toLowerCase().includes(nameTerm));
             if ( 0 == data.length)
             data = person.filter(({ ip_address }) => ip_address.toLowerCase().includes(nameTerm));
     }}
   }
 }
