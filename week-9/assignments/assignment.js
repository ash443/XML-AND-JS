const htmlToElement = (html) => {
  ...
  const renderTable = (xmlData,nameTerm) => {
  ...
  loadData(`http://127.0.0.1:8080/assignments/people.xml`).then((data) => renderTable(data));
  
  const onSubmit = (event) => {
    event.preventDefault();
    const term = event.target.name.value;
    loadData(`http://127.0.0.1:8080/assignments/people.xml`).then((data) => renderTable(data, term));
  };
